...
6- Temos duas peças importantes para organizar os dados dos sensores no meu sistema:

- SensorModel (A Lista de Tipos de Sensores):


-- O que é: É como uma lista predefinida dos "tipos de sensor" que o seu sistema conhece (por exemplo, "Termopar AG", "Termopar AS", "Alta Frequência").
-- Para que serve: Garante que você só use tipos de sensor válidos, evitando erros e deixando o código mais claro sobre qual sensor está sendo usado.

- ISensor (A Identidade de Cada Sensor):

-- O que é: Define exatamente as informações que cada sensor individual deve ter (um ID único, qual é o seu SensorModel, e opcionalmente uma imagem).
